...
func NewUsers(id string, nickname string, email string, password string, name string, lastname string, idType int, idNumber string, cellphone string, birthDate time.Time, verifiedCode string, verifiedAt time.Time, fullPathPhoto string, rsaPrivate string, rsaPublic string, id_role int) *User {
	return &User{
		ID:             id,
		Nickname:       nickname,
		Email:          email,
